...
public class WorldGenMagicTree extends WorldGenAbstractTree {
				...
				if (isSoil && y < 256 - size - 1) {
					...
					for (k2 = 0; k2 < size; ++k2) {
						...
						if (block3.isAir(world, X, y + k2, Z) || block3.isLeaves(world, X, y + k2, Z)) {
							this.setBlockAndNotifyAdequately(world, X, y + k2, Z, GameRegistry.findBlock("derpyshiz", "log"), 1);
						}
							...
							for (int i2 = z - k1; i2 <= z + k1; ++i2) {
								...
								if (Math.abs(l1) != k1 || Math.abs(j2) != k1 || rand.nextInt(2) != 0 && j1 != 0) {
									...
									if (block1.isAir(world, X, k2, Z) || block1.isLeaves(world, X, k2, Z)) {
										this.setBlockAndNotifyAdequately(world, X, k2, Z, GameRegistry.findBlock("derpyshiz", "leaves"), 1);
									}
								}
							}
						}
					...
					genSide(world,x+1,y,z,5);
					genSide(world,x-1,y,z,5);
					genSide(world,x,y,z+1,9);
					genSide(world,x,y,z-1,9);
